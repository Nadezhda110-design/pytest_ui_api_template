...
@allure.feature("UI Поиск на сайте Читай-город")
@allure.story("Поисковая строка")
@pytest.mark.ui
class TestSearch:

    @allure.title("Поиск книги по названию на кириллице")
    @allure.description("Проверяем, что поиск по русскому "
                        "названию возвращает непустой список книг")
    ...
    def test_search_by_russian_title(self, driver, query):
        ...
        matching_titles = [title for title in titles
    ...
    @allure.description("Проверяем, что поиск по английскому "
                        "названию возвращает непустой список")
    @allure.severity(allure.severity_level.CRITICAL)
    ...
    def test_search_by_english_title(self, driver, query):
        ...
        assert len(matching_titles) > 0, \
            f"Результаты {titles} не соответствуют запросу '{query}'"

        print(f"\n✅ По запросу '{query}' найдено книг: {len(titles)}")

    @allure.title("Поиск книг по автору на кириллице")
    @allure.description("Проверяем, что поиск по автору "
                        "возвращает книги этого автора")
    ...
    def test_search_by_russian_author(self, driver, query):
        ...
        assert search_page.has_search_results(), \
            f"Не найдено книг по автору '{query}'"
        ...
        # Если нет прямых совпадений в названиях,
        # проверяем хотя бы наличие результатов
        if len(matching_titles) == 0:
            # Проверяем URL страницы
            current_url = search_page.get_current_url().lower()
            if query_lower in current_url:
                print(f"⚠️ Автор '{query}' найден в URL, "
                      f"но не в названиях книг")
            else:
                # Если автор не найден нигде, но результаты есть
                print(f"⚠️ По автору '{query}' есть результаты, "
                      f"но имя не отображается в названиях")

        print(f"\n✅ По автору '{query}' найдено книг: {len(titles)}")

    @allure.title("Поиск книг по автору на латинице")
    @allure.description("Проверяем, что поиск по автору "
                        "на английском возвращает результаты")
    @allure.severity(allure.severity_level.NORMAL)
    ...
    def test_empty_search_query(self, driver):
        ...
        # Проверяем, что поиск не выполнился
        # (нет перехода на страницу результатов)
        assert ("search" not in current_url.lower()
                or current_url == initial_url), \
            (f"Произошел переход на страницу поиска "
             f"при пустом запросе: {current_url}")
